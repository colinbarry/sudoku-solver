...
const handleClick = num => {
  const { r, c } = store.state.selectedCell;
  store.actions.updateCell(r, c, num);
}

const handleSolve = () => {
  solve(store.state.values);
}

const handleClear = () => {
  store.actions.updateCells([]);
}
